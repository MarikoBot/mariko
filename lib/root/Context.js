...
class Context {
    ...
    transformMessageData(data) {
        ...
        if (data.includes('{{ephemeral:'))
            ephemeral = eval(data.split('{{ephemeral:')[1].split('}}')[0]);
        let finalStr = data.includes('{{color:') ? data.split('{{color:')[0] + data.split('}}')[1] : data;
     ...
     */
    translate(key, ...vars) {
        const str = this.command.client.Languages.getStr(key, this.languageId);
        let finalStr = '';
        if (vars.length > 0) {
            for (let i = 0; i < str.split('{{').length - 1; i++) {
                const piece = str.split('{{')[i];
                const nextPiece = str.split('{{')[i + 1].split('}}')[1];
