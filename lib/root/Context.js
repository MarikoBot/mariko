...
class Context {
    ...
    static extractDataFromStr(str) {
        const data = {};
        let finalStr = str;
        for (let i = 0; i < str.split(ClientConfig_1.default.extBracketsOpen).length - 1; i++) {
            const KV = finalStr.split(ClientConfig_1.default.extBracketsOpen)[1].split(ClientConfig_1.default.extBracketsClose)[0];
            const key = KV.split(':')[0];
            const value = KV.split(':')[1];
            finalStr = finalStr.includes(ClientConfig_1.default.extBracketsOpen)
                ? finalStr.split(ClientConfig_1.default.extBracketsOpen)[0] +
                    (finalStr.includes(ClientConfig_1.default.extBracketsClose) ? finalStr.split('}}')[1] : '')
                : finalStr;
            data[key] = value;
        }
    ...
    async validOrCancelDialog(messageData, buttonsToSet = ['accept', 'decline'], timeout, reply = false, messageToEdit) {
        ...
        const [response, message] = await this.messageComponentInteraction(messageData, [row], timeout, reply, messageToEdit);
        if (!response)
    ...
    async modalDialog(contentToShow, modalData, timeout, customId = 'modal', reply = false, messageToEdit) {
        ...
        let value = 'Rien';
        while (loop) {
            message = message ? (await message.fetch().catch(Util_1.caught)) || message : message || null;
            ...
            if (response.customId.includes('modal'))
                await response.showModal(modal).catch(Util_1.caught);
        }
        if (!response)
    ...
    async messageComponentInteraction(messageData, rows, timeout, reply = false, messageToEdit) {
        ...
        const response = await message.awaitMessageComponent({ filter, time: timeout }).catch(Util_1.caught);
        if (!response)
            return [null, message || null];
    ...
    async send(messageData) {
        if (!this.channel.isTextBased())
            return null;
        const message = await this.channel.send(this.transformMessageData(messageData)).catch(Util_1.caught);
        if (!message)
            return null;
        return message;
    }
    ...
    async reply(messageData, interaction = this.interaction) {
        ...
        try {
            ...
            if (!interaction.deferred) {
                message = await interaction.reply(transformed).catch(Util_1.caught);
            }
            else {
                message = await interaction.followUp(transformed).catch(Util_1.caught);
            }
            if (!message)
                return null;
        }
    ...
    async edit(messageData, message, ignorePresentFields = false) {
        ...
        if ('files' in messageData && messageData.files.length > 0 && !ignorePresentFields) {
            await message.removeAttachments().catch(Util_1.caught);
        }
        if (!message)
            return null;
        const editedMessage = await message.edit(messageData).catch(Util_1.caught);
        if (!editedMessage)
            return null;
    ...
    async loadLanguage() {
        if (this.users.length > 0)
            this.languageId = await this.command.client.Server.User.getLanguage(this.users[0].id);
    }
    /**
