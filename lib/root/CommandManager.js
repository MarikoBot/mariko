...
class CommandManager {
    ...
    getCommand(interaction) {
        let commandToReturn;
        const command = this.commandsList.get(interaction.commandName);
        const thereIsOption = interaction.options.data;
        const thereIsGroup = thereIsOption.filter((elt) => elt.type === v10_1.ApplicationCommandOptionType.SubcommandGroup);
        const thereIsSub = thereIsOption.filter((elt) => elt.type === v10_1.ApplicationCommandOptionType.Subcommand);
