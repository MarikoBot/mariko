    ...
    if (interaction.isChatInputCommand()) {
        ...
        if (interaction.customId.startsWith('blacklist')) {
            ...
            if (interaction.customId === 'blacklist_add') {
                ...
                let finalMessage;
                if (tested.valid) {
                    finalMessage = `<t:${(0, Util_1.discordDate)()}:T> | ${Emojis_1.default.coloredFlag}${Emojis_1.default.coloredCheck} | Object \`${tested.name}\` **successfully** __added__ to the blacklist.`;
                    if (tested.errors.length > 0)
                        finalMessage += `\n\`Warnings:\`\n>>> ${tested.errors.join('\n')}`;
                    await client.Server.Core.blacklistElement(tested);
                }
                else {
                    finalMessage = `<t:${(0, Util_1.discordDate)()}:T> | ${Emojis_1.default.coloredFlag}${Emojis_1.default.coloredCancel} | Object \`${tested.name}\` **cannot be** __added__ to the blacklist.`;
                    if (tested.errors.length > 0)
                        finalMessage += `\n\`Errors:\`\n>>> ${tested.errors.join('\n')}`;
                }
                ...
                let finalMessage;
                if (tested.valid) {
                    finalMessage = `<t:${(0, Util_1.discordDate)()}:T> | ${Emojis_1.default.coloredFlag}${Emojis_1.default.coloredCheck} | Object \`${tested.name}\` **successfully** __removed__ from the blacklist.`;
                    if (tested.errors.length > 0)
                        finalMessage += `\n\`Warnings:\`\n>>> ${tested.errors.join('\n')}`;
                    await client.Server.Core.unblacklistElement(tested.data.get('index').value);
                }
                else {
                    finalMessage = `<t:${(0, Util_1.discordDate)()}:T> | ${Emojis_1.default.coloredFlag}${Emojis_1.default.coloredCancel} | Object \`${tested.name}\` **cannot be** __removed__ from the blacklist.`;
                    if (tested.errors.length > 0)
                        finalMessage += `\n\`Errors:\`\n>>> ${tested.errors.join('\n')}`;
                }
                await interaction.reply({ content: finalMessage, ephemeral: true }).catch(Util_1.clean);
