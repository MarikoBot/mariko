...
class default_1 {
    ...
    async displayPage(page) {
        if (!this.ctx.btn || !this.ctx.btn.message) {
            ...
            return await this.ctx.btn.reply(generatedOptions).catch(Util_1.caught);
        }
    }
    /**
    ...
    async handlePagination(inter) {
        ...
        if (!panel)
            return;
        panel = (await panel.fetch().catch(Util_1.caught));
        if (!panel)
            return;
        ...
        let loop = true;
        while (loop) {
            const collector = await panel.awaitMessageComponent(collectorOptions).catch(Util_1.caught);
            if (!collector) {
                loop = false;
                break;
            }
            ...
            await panel.edit(pageOptions).catch(Util_1.caught);
            await collector.deferUpdate().catch(Util_1.caught);
        }
    }
    /**
    ...
    async displayAddModal(inter) {
        ...
        await inter.showModal(modal).catch(Util_1.caught);
    }
    /**
