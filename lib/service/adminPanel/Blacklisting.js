...
const mainEmbed = async (blacklist, color, page = 0) => {
    ...
    if (blacklist.length < eltPerPage * page || blacklist.length === 0) {
        ...
        for (let i = eltPerPage * page; i < eltPerPage * (page + 1); i++) {
            ...
            embed.addFields({
                name: `${Emojis_1.default.invisibleSquare}\`${i}\``,
...
class Blacklisting {
     ...
     */
    get blacklist() {
        return this.client.Servers.Core.getBlacklist();
    }
    /**
    ...
    async messageOptions(page) {
        return {
            ...
            components: (0, index_1.generatePanelRows)((0, exports.panelButtons)((await this.client.Servers.Core.getBlacklist()).length)),
            ephemeral: true,
        };
    }
    /**
    ...
    async validAddModal(fields, authorId) {
        ...
        if (fetchedType) {
            if (fetchedType !== typeValue) {
                tested.valid = false;
                tested.errors.push(`Type **${typeValue}** cannot be associated with **${fetchedType}** object for \`${idValue}\`.`);
            }
        }
        else
            tested.errors.push(`No **${typeValue}** was found for \`${idValue}\`. This warning can occur when the ${typeValue} is not available by the bot.`);
        if (!ClientConfig_1.default.commandsListRegexp.test(commandsValue)) {
            tested.valid = false;
            tested.errors.push(`\`${commandsValue}\` string must follow the below rule:\`\`\`fix\n${ClientConfig_1.default.commandsListRegexp}\`\`\`` +
        ...
        const blacklist = await this.client.Servers.Core.getBlacklist();
        if (blacklist.map((e) => e.id).includes(idValue)) {
            tested.valid = false;
            tested.errors.push(`Object \`${tested.name}\` is already blacklisted. If you want to change informations, remove it before.`);
        }
    ...
    async validRemoveModal(fields) {
        ...
        if (Number(indexValue) > blacklistLength) {
            tested.valid = false;
            tested.errors.push(`There aren't as many elements in the blacklist. \`${indexValue}\` is out of range \`${0}-${blacklistLength - 1}\`.`);
        }
        tested.name = blacklist[Number(indexValue)].id;
        return tested;
    }
}
exports.default = Blacklisting;
