...
const mainEmbed = async (blacklist, color, page = 0) => {
    ...
    if (blacklist.length < eltPerPage * page || blacklist.length === 0) {
        ...
        for (let i = eltPerPage * page; i < eltPerPage * (page + 1); i++) {
            ...
            embed.addFields({
                name: Emojis_1.default.invisibleSquare,
...
class Blacklisting {
    ...
    async messageOptions(page) {
        return {
            ...
            components: (0, Monitoring_1.generatePanelRows)((0, exports.panelButtons)((await this.client.Servers.Core.getBlacklist()).length)),
            ephemeral: true,
        };
    }
    /**
