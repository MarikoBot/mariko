...
class default_1 {
    ...
    async validAddModal(fields) {
        ...
        const fetchedThing = (await (0, Util_1.IdToGuild)(this.client, typeValue)) || (await (0, Util_1.IdToUser)(this.client, typeValue)) || false;
        const fetchedType = fetchedThing ? fetchedThing.constructor.name : null;
        if (fetchedThing)
        ...
        if (fetchedType) {
            ...
            tested.errors.push(`${Emojis_1.default.coloredWarning} No **${typeValue}** was found for \`${idValue}\`. Please be sure it exists, I can't check.`);
        if (!ClientConfig_1.default.commandsListRegexp.test(commandsValue)) {
            tested.valid = false;
            tested.errors.push(`${Emojis_1.default.coloredWarning} \`${commandsValue}\` string must follow the below rule:\`\`\`fix\n${ClientConfig_1.default.commandsListRegexp}\`\`\`` +
