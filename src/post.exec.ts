 ...
 * @returns The embeds.
 */
export function renderWebhooksEmbeds(content: string): EmbedBuilder[] {
  const embeds: EmbedBuilder[] = [];

  if (content === 'dojo_rules') {
    embeds.push(
      new EmbedBuilder().setImage(
        'https://cdn.discordapp.com/attachments/1012372287640567948/1159409615755300884/Rules.png?ex=6530eb59&is=651e7659&hm=6cc8d5ce8131f263357a763559e8d57ff51a3d1b94cda1db4b5d571cc828b851&',
      ),
      new EmbedBuilder().setDescription(
        '### Le règlement de ce serveur est à lire, à comprendre et à accepter. En cas de litige, nous nous appuierons sur ce dernier. Nous rappelons également que tout membre de ce serveur se doit de respecter les ToS de Discord.',
      ),
      new EmbedBuilder().setTitle('I. 🍡 Tenue Générale').setFields(
        {
          name: 'Votre profil doit être conforme.',
          value:
            '> Il ne doit pas offenser qui que ce soit, doit être facilement identifiable. Ceci interdit également le contenu pornographique et choquant.\n᲼',
        },
        {
          name: 'Veuillez avoir un comportement respectueux envers les autres membres.',
          value:
            "> Les insultes et le contenu choquant ne sont pas autorisés. Toute forme d'irrespect est interdit. L'incitation à la haine n'est pas autorisée. Veuillez ne pas faire de nuisance sonore dans les salons vocaux.\n᲼",
        },
        {
      ...
      new EmbedBuilder().setTitle("II. 💵 L'économie (UnbelievableBoat)").setFields(
        {
          name: "L'alting n'est pas autorisé.",
          value:
            "> Seulement un compte peut servir à jouer par personne. Les transferts d'argent vers un alt sont interdits.\n᲼",
        },
        {
        ...
        },
        {
          ...
          value:
            "> Le fait de faire partie d'une alliance ne vous protège pas contre les potentiels vols de la part des gens de votre alliance. Aucune organisation n'est officielle et aucune n'est protégée par des quelconques règles.\n᲼",
        },
      ),
      new EmbedBuilder()
        .setTitle('III. 🀄 Mudae')
        .setDescription(
          "**Il est interdit de claim un personnage provenant des rolls de quelqu'un d'autre.**\n" +
        ...
        .setFields(
          {
            ...
            value:
              "> N'hésitez pas à demander si le salon est disponible si nécessaire. Indiquez quand vous faites vos rolls et indiquez quand vous les terminez si vous savez que d'autres personnes attendent leur tour.\n᲼",
          },
          {
            name: "L'alting n'est pas autorisé.",
            value:
              "> Seulement un compte peut servir à jouer par personne. Les transferts de personnages vers un alt sont interdits. Peu importe le fond ou la forme, l'utilisation d'un double compte est interdite.\n᲼",
          },
        ),
      new EmbedBuilder()
        .setTitle('IV. 🔑 Les sanctions.')
        .setDescription(
          "> Lorsqu'il est nécessaire, un timeout sera la solution la plus priorisée en cas de mauvais comportement. Une fois la discussion terminée, des sanctions supplémentaires peuvent s'appliquer ou non.\n\n" +
  ...
  for (const embed of embeds) embed.setColor(Colors.DARK);
...
export async function webhookSend(
  ...
  channelId: Snowflake,
  guildId: Snowflake,
  messageContent: WebhookMessageCreateOptions,
): Promise<void> {
  const channel: ContextChannel = (await IdToCtxChannel(client, guildId, channelId)) as TextChannel;
  ...
  await webhook.send(messageContent).catch(clean);
}
